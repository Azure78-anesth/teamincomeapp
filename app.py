...
with tab2:
    ...
    with tab_invoice:
        ...
        # 연도 선택
        base_years = {NOW_KST.year, NOW_KST.year - 1}
        years_avail = sorted({int(x["ym"].split("-")[0]) for x in inv if isinstance(x.get("ym"), str)} | base_years)
        y = st.selectbox(
            "연도 선택",
            years_avail,
...
with tab3:
    ...
    def sb_get_latest_month_before(ym_key: str):
        """현재 ym_key(YYYY-MM)보다 이전에 저장된 가장 최근 월 설정을 가져온다.
        ym_key는 문자열이지만 'YYYY-MM' 포맷이면 문자열 정렬이 시간 순서와 동일하다.
        """
        try:
            res = (
                sdb.table("settlement_month")
                .select("*")
                .lt("ym_key", ym_key)
                .order("ym_key", desc=True)
                .limit(1)
                .execute()
            )
            data = getattr(res, "data", None) or []
    ...
    if not mrow:
        # 해당 월 설정이 없으면, 직전 월에 저장된 설정을 기본으로 복사한다.
        # (신규 월/연도 전환 시 수령자/고정액이 임의값으로 바뀌어 정산이 깨지는 것을 방지)
        prev = sb_get_latest_month_before(ym_key)
        if prev:
            bs = prev.get("receiver_busansoom", "")
            am = prev.get("receiver_amiyou", "")
            sf = int(prev.get("sungmo_fixed", 1000) or 1000)
        else:
            bs = members_all[0] if members_all else ""
            am = members_all[0] if members_all else ""
            sf = 1000
        sb_upsert_month(ym_key, sf, bs, am)
        mrow = sb_get_month(ym_key)
...
with tab4:
    ...
    with tab6_input:
        ...
        years_avail_all = sorted(years_from_inv | {NOW_KST.year, NOW_KST.year - 1})
