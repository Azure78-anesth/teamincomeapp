...
years_range = list(range(this_year, this_year - 5, -1))
...
# ============================
# Income (incomes) - year load + pagination (PostgREST 1000 rows limit 대응)
# ============================
def get_income_year_options() -> list[int]:
    """incomes.date(YYYY-MM-DD) 기준으로 데이터가 존재할 법한 연도 목록을 만들어 반환"""
    years: set[int] = set()
    try:
    ...
    # 세션에 이미 로드된 데이터가 있으면 거기서도 연도 확보
    try:
    ...
    def _year_from_date_str(s: str) -> int | None:
        try:
            s = str(s or "")
    ...
    try:
        ...
        dmin = None
        dmax = None
        if getattr(rmin, "data", None):
            dmin = (rmin.data or [{}])[0].get("date")
        if getattr(rmax, "data", None):
            dmax = (rmax.data or [{}])[0].get("date")

        y0 = _year_from_date_str(dmin)
        y1 = _year_from_date_str(dmax)

        if y0 is not None and y1 is not None:
            for y in range(min(y0, y1), max(y0, y1) + 1):
                years.add(int(y))
        else:
            # 최소/최대가 없으면(테이블 비었거나 오류) 세션/현재연도만 사용
            pass
    except Exception:
        pass
...
def _sb_select_paged(query_builder, batch_size: int = 1000) -> list[dict]:
    """Supabase(PostgREST) 1000개 제한을 range()로 우회해서 전부 가져오기"""
    rows: list[dict] = []
    offset = 0
    while True:
        q = query_builder.range(offset, offset + batch_size - 1).execute()
...
def load_incomes_year(year: int) -> None:
    """선택한 연도의 incomes만 로드해서 st.session_state.income_records를 갱신"""
    init_state()
    if not sb:
        st.session_state["_income_loaded_year"] = year
        return
    y0 = int(year)
    y1 = y0 + 1
    start_date = f"{y0:04d}-01-01"
    end_date = f"{y1:04d}-01-01"
    try:
        qb = (
            sb.table("incomes")
              .select("*")
        ...
        incs = _sb_select_paged(qb, batch_size=1000)
        st.session_state.income_records = [{
            "id": x["id"], "date": x["date"],
        ...
        st.session_state["_income_loaded_year"] = y0
    except Exception as e:
        st.warning(f"수입 데이터 로드 실패: {e}")
...
def upsert_row(table: str, payload: Dict[str, Any]):
    if sb:
        try:
            ...
            if table == "incomes":
                try:
                    y = int(str(payload.get("date",""))[:4])
                    load_incomes_year(y)
                except Exception:
                    load_incomes_year(NOW_KST.year)
            return
        except Exception:
    ...
    if sb:
        try:
            ...
            try:
                ...
                y = int(st.session_state.get("_income_loaded_year", NOW_KST.year))
            load_incomes_year(y)
            return
        except Exception:
...
def load_invoices(year: int | None = None):
    ...
    try:
        ...
        try:
            ...
            pass
        rows = _sb_select_paged(q, batch_size=1000)
...
with tab2:
    ...
    # ── 연도 선택 (DB 기반 옵션)
    years = get_income_year_options()
    cur_year = NOW_KST.year
    default_year = cur_year if cur_year in years else (years[-1] if years else cur_year)
    c1, c2 = st.columns([3,2])
    with c1:
        year = st.selectbox('연도(연간 리셋/독립 집계)', years, index=years.index(default_year) if (years and default_year in years) else 0, key='stat_year')
    with c2:
        st.caption('선택 연도 외 데이터는 저장만 유지(열람 전용)')

    # ✅ 선택한 연도 수입 데이터를 DB에서 다시 로드 (1000개 제한 방지: 페이지네이션)
    try:
        loaded_y = st.session_state.get("_income_loaded_year")
        if loaded_y != year:
            load_incomes_year(year)
    except Exception:
        pass
...
with tab5:
    ...
    # ── 연도 선택 (사이드바 없이, 이 탭의 연도 선택값 사용)
    years_all = get_income_year_options()
    cur_year = NOW_KST.year
    default_year = st.session_state.get("records_year", cur_year)
    if default_year not in years_all:
        default_year = cur_year if cur_year in years_all else (years_all[-1] if years_all else cur_year)

    year_sel = st.selectbox("연도", years_all, index=years_all.index(default_year) if (years_all and default_year in years_all) else 0, key="records_year")

    # ✅ 선택 연도 데이터 로드(1000개 제한 방지) — 연도 변경 시 즉시 반영
    try:
        loaded_y = st.session_state.get("_income_loaded_year")
        if loaded_y != year_sel:
            load_incomes_year(year_sel)
            st.rerun()
    except Exception:
        pass
    ...
    records = st.session_state.get("income_records", [])
    if not records:
        st.info(f"{year_sel}년 데이터가 없습니다. 다른 연도를 선택해 보세요.")
        st.stop()
    ...
    with c1: st.caption(f"연도: {year_sel}")
...
with tab3:
    ...
    # ───────── 원천 수입 (연도 선택 + DB 페이지네이션 로드) ─────────
    years = get_income_year_options()
    cur_year = NOW_KST.year
    default_year = cur_year if cur_year in years else (years[-1] if years else cur_year)

    year = st.selectbox("정산 연도", years, index=years.index(default_year) if (years and default_year in years) else 0, key="settle_year")

    # ✅ 선택한 연도 수입 데이터를 DB에서 다시 로드 (1000개 제한 방지)
    try:
        loaded_y = st.session_state.get("_income_loaded_year")
        if loaded_y != year:
            load_incomes_year(year)
    ...
    if not rec:
        st.info(f"{year}년 수입 데이터가 없습니다. (다른 연도를 선택하거나 [수입 입력]에서 추가해주세요.)")
        st.stop()
...
with tab4:
    ...
    def reload_invoice_records(year: int) -> None:
        ...
        try:
            ...
            rows = _sb_select_paged(qb, batch_size=1000)
