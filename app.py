...
# ============================
# Incomes: 연도별 로드(페이지네이션) + 캐시
# ============================
def _parse_year_from_date(v) -> int | None:
    try:
        s = str(v)
...
def get_income_year_options() -> list[int]:
    """incomes.date 기준으로 조회 가능한 연도 옵션을 만든다(최소/최대 연도 범위)."""
    years: set[int] = set()
    try:
    ...
    # 이미 세션에 로드된 값에서 보강
    for r in (st.session_state.get("income_records") or []):
        y = _parse_year_from_date(r.get("date"))
        if y: years.add(y)

    if sb:
        try:
            rmin = sb.table("incomes").select("date").order("date", desc=False).limit(1).execute()
            rmax = sb.table("incomes").select("date").order("date", desc=True).limit(1).execute()
            min_y = _parse_year_from_date(((rmin.data or [{}])[0]).get("date")) if hasattr(rmin, "data") else None
            max_y = _parse_year_from_date(((rmax.data or [{}])[0]).get("date")) if hasattr(rmax, "data") else None
...
def _fetch_incomes_year_from_sb(year: int, batch_size: int = 1000) -> list[dict]:
    """Supabase에서 해당 연도의 incomes 전체를 batch로 가져온다."""
    if not sb:
        return []
    start_date = f"{year}-01-01"
    end_date   = f"{year+1}-01-01"  # lt 경계
    rows: list[dict] = []
    offset = 0
    while True:
        q = (
...
def load_incomes_for_year(year: int, force: bool = False, set_active: bool = True):
    """세션에 연도별 incomes를 로드한다.

    - force=True  : 캐시가 있어도 DB에서 다시 로드
    - set_active=True : active_income_year/income_records를 해당 연도로 전환
    """
    st.session_state.setdefault("incomes_by_year", {})
    y = int(year)

    if (not force) and (y in st.session_state["incomes_by_year"]):
        if set_active:
            st.session_state.active_income_year = y
            st.session_state.income_records = st.session_state["incomes_by_year"][y]
        return

    if sb:
        incs = _fetch_incomes_year_from_sb(y)
        mapped = [{
            "id": x["id"], "date": x["date"],
        ...
        # 메모리 모드: 기존 income_records를 연도 필터한 결과로 맞춤
        rec = st.session_state.get("income_records", []) or []
        mapped = [r for r in rec if _parse_year_from_date(r.get("date")) == y]

    st.session_state["incomes_by_year"][y] = mapped
    if set_active:
        st.session_state.active_income_year = y
        st.session_state.income_records = mapped

def _on_income_year_change(key_name: str):
    try:
        y = int(st.session_state.get(key_name))
    except Exception:
        return
    st.session_state.records_page = 0
    load_incomes_for_year(y, force=False)
...
def load_data():
    if sb:
        try:
            ...
            # incomes는 연도별로 로드(페이지네이션 + 연도 필터)
            st.session_state.setdefault("active_income_year", int(NOW_KST.year))
            load_incomes_for_year(int(st.session_state["active_income_year"]), force=True)
        except Exception:
...
with tab2:
    ...
    # ── (FIX) Supabase 1000개 제한 회피: 연도 옵션 + 연도별 로드
    years_range = get_income_year_options()
    cur_year = int(NOW_KST.year)
    if not years_range:
        years_range = [cur_year]
    default_year = int(st.session_state.get("stat_year", st.session_state.get("active_income_year", cur_year)))
    if default_year not in years_range:
        default_year = years_range[-1]
    ...
    with c1:
        year = st.selectbox(
            '연도(연간 리셋/독립 집계)',
            years_range,
            index=years_range.index(default_year),
            key='stat_year',
            on_change=_on_income_year_change,
            args=('stat_year',)
        )
    with c2:
        st.caption('선택 연도 외 데이터는 저장만 유지(열람 전용)')

    load_incomes_for_year(int(year), force=False, set_active=True)
    ...
    dfY = df[df['year'] == int(year)].copy()
    if dfY.empty:
        st.warning(f'{int(year)}년 데이터가 없습니다.')
        st.stop()
    ...
    with tab_loc_all:
        ...
        # ✅ 연간/월간 모두에서 과거 연도 선택 가능
        cA, cB = st.columns([2, 3])
        with cA:
            year_loc_all = st.selectbox('연도', years_range, index=years_range.index(year), key='loc_all_year')
        with cB:
            cat_sel = st.radio('분류 선택', ['보험','비보험'], horizontal=True, key='loc_all_cat')

        dfC = df[(df['year'] == year_loc_all) & (df['category'] == cat_sel)].copy()

        if dfC.empty:
            st.warning(f'{year_loc_all}년 {cat_sel} 데이터가 없습니다.')
        else:
    ...
    with tab_loc_each:
        ...
        cA, cB = st.columns([2, 3])
        with cA:
            year_loc_each = st.selectbox('연도', years_range, index=years_range.index(year), key='loc_each_year')
        with cB:
            # 1) 분류(보험/비보험)
            cat_sel_e = st.radio('분류 선택', ['보험', '비보험'], horizontal=True, key='loc_each_cat')

        dfC_e = df[(df['year'] == year_loc_each) & (df['category'] == cat_sel_e)].copy()
        if dfC_e.empty:
            st.warning(f"{year_loc_each}년 {cat_sel_e} 데이터가 없습니다.")
            st.stop()
...
with tab5:
    ...
    # ── (FIX) Supabase 1000개 제한 회피: 연도 옵션 + 연도별 로드(기록 관리)
    years_opt = get_income_year_options()
    cur_year = int(NOW_KST.year)
    if not years_opt:
        years_opt = [cur_year]
    desired_year = int(st.session_state.get("manage_year", st.session_state.get("active_income_year", cur_year)))
    if desired_year not in years_opt:
        desired_year = years_opt[-1]
    load_incomes_for_year(int(desired_year), force=False, set_active=True)
    ...
    with c1: year_sel = st.selectbox("연도", years_opt, index=years_opt.index(int(desired_year)), key="manage_year", on_change=_on_income_year_change, args=("manage_year",))
    if int(year_sel) != int(desired_year):
        load_incomes_for_year(int(year_sel), force=False, set_active=True)
        st.rerun()
...
with tab3:
    ...
    # ── (FIX) Supabase 1000개 제한 회피: 정산 연도 옵션 + 연도별 로드
    years_opt = get_income_year_options()
    cur_year = int(NOW_KST.year)
    if not years_opt:
        years_opt = [cur_year]
    default_year = int(st.session_state.get("settle_year", st.session_state.get("active_income_year", cur_year)))
    if default_year not in years_opt:
        default_year = years_opt[-1]

    year = st.selectbox(
        "정산 연도",
        years_opt,
        index=years_opt.index(default_year),
        key="settle_year",
        on_change=_on_income_year_change,
        args=("settle_year",)
    )
