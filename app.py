...
# ============================
# Invoice Year Options (계산서 연도 선택용)
# ============================
def get_invoice_year_options(client, current_year: int, fallback_records: List[Dict[str, Any]] | None = None) -> List[int]:
    """계산서(인보이스) 연도 선택 옵션을 안정적으로 생성.

    목표:
      - 2025 같은 과거 연도도 선택 가능
      - 앞으로 데이터가 쌓이면(예: 2027년) 2026/2025도 계속 조회 가능

    방식:
      - Supabase 사용 시 invoices.ym의 최소/최대만 조회해서 연도 범위를 생성(대량 데이터에도 안전)
      - 오류/오프라인 시 현재 세션의 invoice_records에서 연도를 추출
    """
    years: set[int] = set()

    def _parse_year(ym_val: Any) -> int | None:
        try:
            s = str(ym_val)
            if len(s) >= 4:
                return int(s[:4])
    ...
    # 1) DB에서 최소/최대 연도로 범위 생성
    if client is not None:
        try:
            q_min = client.table("invoices").select("ym").order("ym", desc=False).limit(1).execute()
            q_max = client.table("invoices").select("ym").order("ym", desc=True).limit(1).execute()
            ym_min = (q_min.data or [{}])[0].get("ym") if getattr(q_min, "data", None) is not None else None
            ym_max = (q_max.data or [{}])[0].get("ym") if getattr(q_max, "data", None) is not None else None
            min_y = _parse_year(ym_min)
            max_y = _parse_year(ym_max)
            if min_y is not None and max_y is not None:
                hi = max(max_y, int(current_year))
                for y in range(min_y, hi + 1):
                    years.add(int(y))
        except Exception:
            pass

    # 2) 실패 시 세션 데이터에서 보강
    if not years and fallback_records:
        try:
            for r in fallback_records:
                y = _parse_year(r.get("ym"))
                if y is not None:
                    years.add(y)
        except Exception:
            pass
...
with tab2:
    ...
    with tab_invoice:
        ...
        # 연도 선택 (DB의 최소/최대 연도 기반으로 범위 생성 → 2027년에도 2026/2025 조회 가능)
        years_avail = get_invoice_year_options(sb, NOW_KST.year, inv)
        y = st.selectbox(
            "연도 선택",
            years_avail,
...
with tab4:
    ...
    with tab6_input:
        ...
        # 연/월
        # 연도 선택 (DB의 최소/최대 연도 기반으로 범위 생성 → 2027년에도 2026/2025 입력/조회 가능)
        years_avail_all = get_invoice_year_options(_sb, NOW_KST.year, ss.invoice_records)
    ...
    with tab6_manage:
        ...
        if ss.edit_invoice_id:
            ...
            if target:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    years_all = sorted({int(x["ym"][:4]) for x in ss.invoice_records if x.get("ym")} | {NOW_KST.year})
