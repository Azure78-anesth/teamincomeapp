...
with tab2:
    ...
    # 연도 범위(최초~현재/최신) — 업체종합/업체개별에서 과거연도 선택용
    min_year = int(min(years + [cur_year])) if years else int(cur_year)
    max_year = int(max(years + [cur_year])) if years else int(cur_year)
