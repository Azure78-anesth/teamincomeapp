...
with tab2:
    ...
    with tab_mem:
        ...
        present_members = set(dfY['member'].dropna().unique().tolist())
        members = [m.get('name') for m in st.session_state.team_members if m.get('name') in present_members]
    ...
    with tab_invoice:
        ...
        if mem == "팀 전체":
            ...
            if not Q:
                ...
                if not df_mem.empty:
                    _member_order = {m.get("name"): i for i, m in enumerate(st.session_state.get("team_members", []) or [])}
                    df_mem["_order"] = df_mem["팀원"].map(_member_order).fillna(999999)
                    df_mem = df_mem.sort_values("_order", ascending=True).drop(columns=["_order"]).reset_index(drop=True)
                    st.dataframe(
                        df_mem,
