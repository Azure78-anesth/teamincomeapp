...
with tab3:
    ...
    with st.form("add_member_form", clear_on_submit=True):
        ...
        if st.form_submit_button("팀원 추가"):
            if new_member.strip():
                mid = f"m_{datetime.utcnow().timestamp()}"  # 내부용 ID (UI에 표시 안 함)
    ...
    if st.session_state.team_members:
        ...
        with h1: st.caption("이름")
        with h2: st.caption("위로")
        with h3: st.caption("아래로")
        with h4: st.caption("삭제")

        # 행 렌더
        for i, m in enumerate(tm):
            c1, c2, c3, c4 = st.columns([6, 1, 1, 1])
            with c1:
                # ✅ 이름만 보이게 (ID는 절대 표시하지 않음)
                st.write(f"**{m['name']}**")
            with c2:
                if st.button("▲", key=f"member_up_{m['id']}", disabled=(i == 0)):
                    swap_order("team_members", i, i-1)  # 내부적으로 order만 교체
            with c3:
    ...
    with st.form("add_location_form", clear_on_submit=True):
        ...
        if st.form_submit_button("업체 추가"):
            if loc_name.strip():
                lid = f"l_{datetime.utcnow().timestamp()}"  # 내부용 ID (UI에 표시 안 함)
    ...
    if st.session_state.locations:
        ...
        with h1: st.caption("업체명")
        with h2: st.caption("분류")
        with h3: st.caption("위로")
        with h4: st.caption("아래로")
        with h5: st.caption("삭제")

        if not filtered:
            st.info(f"'{cat_view}' 분류에 등록된 업체가 없습니다.")
        else:
            # 선택 분류 내에서만 상대 이동되도록 보이는 순서 k 기준 처리
            for k, (i_master, l) in enumerate(filtered):
                c1, c2, c3, c4, c5 = st.columns([5.5, 2, 1, 1, 1])
                with c1:
                    st.write(f"**{l['name']}**")  # ✅ 이름만 표시
                with c2:
                    st.write(l.get("category", ""))
                with c3:
                    if st.button("▲", key=f"loc_up_{l['id']}", disabled=(k == 0)):
                        # k 인접 교환 → 실제 저장은 마스터 인덱스끼리 교환
                        i_master_to = filtered[k-1][0]
                        swap_order("locations", i_master, i_master_to)
                with c4:
